...
[![Release](https://img.shields.io/badge/Release-v1.3.0-5B8CFF)](#-下载与安装)
...
- **跟随上游 0.3.0**：代理模式运行库（兼容性更好，游戏对 NGX 的调用不变）、帧倍率上限提升至 6X（游戏支持动态插帧时）、安装包已代码签名
...
前往 [**Releases**](../../releases) 页面下载对应版本（当前 **v1.3.0**）：

| 版本 | 文件 | 适合人群 |
|---|---|---|
| **免安装便携版** | `DLSSG_Manager_1.3.0_portable_x64.zip` | 想即解压即用、绿色不写注册表 |
| **标准安装版** | `DLSSG_Manager_1.3.0_setup_x64.exe` | 想要安装向导、开始菜单/桌面快捷方式与完整卸载 |
...
| 显卡 | NVIDIA RTX 30 系（SM86）—— 上游 0.3.0 起出厂配置仅覆盖 SM86；RTX 20 系请使用旧版工具 v1.2.0（payload 0.2.4） |
...
- 跟随上游 0.3.0：帧倍率上限 6X（游戏支持动态插帧时；多数游戏为 4X 上限，由游戏侧插件决定）
- 上游 0.3.0 实卡验证为 RTX 3080 Ti / 3070；SM75 实验路由的内核族已并入，但出厂配置暂未覆盖 RTX 20 系
